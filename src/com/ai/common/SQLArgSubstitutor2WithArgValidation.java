 ...
 */
public class SQLArgSubstitutor2WithArgValidation extends SQLArgSubstitutor2Updated implements IInitializable 
{
   public static void main(String[] args)
   {
      com.ai.application.defaultpkg.ApplicationHolder.initApplication(args[0],args);
      SQLArgSubstitutor2Updated s = new SQLArgSubstitutor2WithArgValidation();
   ...
   throws DBException
   {
	   ...
	   if (StringUtils.isEmpty(nonStringFieldValue))
	   {
		   ...
		   AppObjects.error(this, "Invalid SQL argument. Empty value. Field name:%s", fieldName);
		   throw new SQLInjectionException("Invalid SQL argument due to empty value passed in. Field Name: " + fieldName);
	   }
	   ...
	   {
		   //it is null case
		   AppObjects.error(this, "Invalid SQL argument. Field Name: %s. String value of null passed: %s"
				   , fieldName
				   , nonStringFieldValue );
	   ...
	   {
		   AppObjects.error(this, "Invalid SQL argument. Field Name: %s. Integer length exceeded (%s/%s). String is %s"
				   ,fieldName
				   ,nonStringFieldValue.length()
				   ,nonStringFieldMaxLength
				   ,nonStringFieldValue );
		   throw new SQLInjectionException("Invalid SQL argument due to length. Input field value:" + nonStringFieldValue);
	   }
	   // it is not empty
	   // it is not null
	   // Length is valid
	   ...
	   if (StringUtils.isNotANumber(nonStringFieldValue))
	   {
		   ...
		   AppObjects.error(this, "Invalid SQL argument. FieldName: %s. Not a number %s"
				   ,fieldName
				   ,nonStringFieldValue );
		   throw new SQLInjectionException("Invalid SQL argument due to number format error. Input field value:" + nonStringFieldValue);
	   }
	   // it is not empty
	   // it is not null
	   // Length is valid
   ...
   throws DBException
   {
	   if (StringUtils.isEmpty(fieldValue))
	   {
		   ...
		   AppObjects.warn(this, "Field value is empty. FieldName: %s. Field type is %s", fieldName, fieldType);
	   }
   }
}//eof-class