 ...
 * 11/30/15
 * *****************
 * Based on SQLArgSubstitutor and SQLArgSubstitutor1
 * Updated on using IDictionary as opposed to a Hashtable
 * Use this hierarchy going forward
 * 
 * @See SQLArgSubstitutor
 * @See SQLArgSubstitutor1
 * @See ISQLArgSubstitutorOptimizedTag
 * 
 * Previous notes
 * **********************
 ...
 */
public class SQLArgSubstitutor2Updated extends CArgSubstitutor
implements ISQLArgSubstitutorOptimizedTag
{
   @Override
   ...
   {
	   return substitute(inString, new MapDictionary(stringArguments), separatorString, escapeCharacter);
   }
   
   /*
    * Everything gets rerouted here
    */
   @Override
   public String substitute(String inString, 
                     ...
                     String escapeCharacter )
   {
	   try
	   {
		   AppObjects.info(this, "Before Substitution: %s", inString);
		   String substitutedString = substituteWithExceptions(inString, stringArguments, separatorString, escapeCharacter);
		   AppObjects.info(this, "After Substitution: %s", substitutedString);
		   return substitutedString;
	   }
	   catch(DBException x)
	   {
		   AppObjects.error(this,"Exception while translating statement:%s", inString);
         ...
         for(;tokenizer.hasMoreTokens();i++)
         {
            ...
            else
            {
               ...
               if (fieldParser.getFieldType() == null)
               {
                  ...
                  validateNonStringField(curToken, value);
               ...
               else
               {
                  ...
                  if (translator != null)
                  {
                	 ...
                	 String valueToAppend = translate(translator, 
                			 fieldParser.getFieldName(),
                			 fieldParser.getFieldType(),
                			 value);
                	 validateTypedStringField(fieldParser.getFieldName(), fieldParser.getFieldType(), valueToAppend);
                  ...
                  else
                  {
                     ...
                     validateNonStringField(fieldParser.getFieldName(),value);
                     buf.append(value);
                  }
               }                  
            }
   ...
   private String translate(ITranslator translator, String fieldName, String fieldType, String fieldValue)
   {
	   if (translator instanceof ITranslator2)
	   {
		   return ((ITranslator2)translator).translateString(fieldName, fieldValue, fieldType);
	   }
	   if (translator instanceof ITranslator1)
	   {
		   return ((ITranslator1)translator).translateString(fieldValue, fieldType);
	   }
	   return translator.translateString(fieldValue);
   }
   ...
   protected void validateNonStringField(String fieldName, String nonStringFieldValue)
   throws DBException
   {
   ...
   protected void validateTypedStringField(String fieldName, String fieldType, String fieldValue)
   throws DBException
   {
	   //none here in this class. for backward compatibility
   }
