 ...
 * Highly experimental
 * 12/31/2010
 * The default webapp and akc webapp are overwriting each other
 * This file is to append the logs as opposed to overwrite
 * I suppose I could use it sparingly for now
 * 
 * with out this for now I may not see log messages from one of the 
 * contexts (may be in the begining). It is all a bit
 * confusing with out further analysis
 * 
 * there is no good way to know which app context due to the way
 * servlet spec is defined. I suppose they haven't anticipated 
 * pig headed way I am running the two contexts pointing to the
 * same root directory.
 */
public class CLog1 implements ILog1, ICreator
{
   ...
   private boolean      m_initialized = false;
    ...
    public CLog1(IConfig config)
    {
      init(config);
    }

	static public FileOutputStream getFileStream( String fileName, IConfig cfg)
      throws CommonException
	{
      ...
      try
      {
         FileOutputStream fos = new FileOutputStream(relativeHtmlFilename, true);
         if (fos != null)
         {
      ...
      try
      {
         FileOutputStream fos = new FileOutputStream(finalFilename,true);
         if (fos != null)
         {
