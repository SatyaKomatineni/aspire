...
public class AKCDisplayParamFilterRequest extends AParamFilterRequest
{
	
	public AKCDisplayParamFilterRequest(HttpServletRequest inRequest)
	{
		super(inRequest);
	}
	...
	throws RequestExecutionException
	{
		try
		{
			IAuthentication2 authObject =
				(IAuthentication2)
				(AppObjects.getIFactory().getObject
				    (AspireConstants.AUTHENTICATION_OBJECT,null));
			return authObject.isAPublicURL(request,null);
		}
		catch (AuthorizationException x)
		{
			throw new RequestExecutionException("authorization exception",x);
		}
	}
}//eof-class