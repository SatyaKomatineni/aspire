...
public class CopyParametersIfNotAvailablePart extends AFactoryPart
{
       ...
       try
       {
         ...
         // put it under the new name as well
          ...
          for(Enumeration e=vOrig.elements();e.hasMoreElements();i++)
          {
             ...
             if (origValue == null)
             {
                AppObjects.warn(this,"Warn:value not found for key:" + origName);
                continue;
             }
             String newName = (String)vNew.elementAt(i);
             String lowercasedNewName = newName.toLowerCase();
             Object newNameValue = inArgs.get(lowercasedNewName);
             if (newNameValue != null)
             {
             	//the new name value exists
             	//dont disturb it
             	continue;
             }
             inArgs.put(lowercasedNewName,origValue);
          }
