...
/**
 * Main property: Allow multiple evaluation of loops if required
 *
 * Naming reverted back to serial numbers
 * Takes the place of AITransform4 superceding AITransform3
 *
 * The following is what the AITransform3 does
 * ***********************************************
 ...
 *
 * In addition this translator enabled transform will indicate when the tags begin and end
 *
 */
public class AITransform5 implements IAITransform, ICreator, ISingleThreaded
{

   public static void processHtmlPage( String htmlFilename
                                        ,PrintWriter writer
                                        , IFormHandler formHandler
                                        , IBooleanExpressionEvaluator evaluator
                                        , ITagListener tagListener
                                        )
        throws java.io.IOException
      {
        AITransform5 parser = new AITransform5(evaluator,tagListener);
    ...
    * If no listener is specificed, an empty listener is used
    */
   private ITagListener m_tagListener = null;

   public AITransform5(IBooleanExpressionEvaluator evaluator
                                    ,ITagListener listener)
   {
      ...
      // initialize the tag listener
      if (listener != null)
      {
         m_tagListener = listener;
      }
      else
      {
         m_tagListener = new EmptyTagListener();
      }
  ...
  // begin with an outside loop state
      ...
      writeToStream(this.getTagListener().startPage());
...
//****************************
                        // move past this tag
                        beginIndex = curRLFTag.getTagEndIndex();
                        writeToStream(getTagListener().tagDetected(curRLFTag,m_parserState));
...
class RLFTag1 implements ITag
{
        ...
        String getGivenName()
        {
               ...
               // givenname is null
                if (getTagName().equals(ITag.TAGS_BGN_LOOP_TAG)
                     || getTagName().equals(ITag.TAGS_END_LOOP_TAG) )
                {
